...
function AuthProvider(props) {
  ...
  const login = async (data) => {
    ...
    const result = await axios.post("http://localhost:4000/auth/login", data); // go to Server : auth.js  for check username & password  and Gen Token
    const token = result.data.token; // Get Token back form auth.js form server
    localStorage.setItem("token", token); //save token at localStorage on web browser
